...
## 페이지 — V5 5탭 (레포 루트 = Pages 루트)

| 탭 | 파일 | 내용 |
|---|---|---|
| 한눈에 보기 | `index.html` | KPI 4종 · 분석 흐름 7단 · 다섯 가지 핵심 결과 · 제도 조합(R0–R3) 막대 |
| 후보지 찾기 | `finder.html` | 대규모 후보지 25곳 지도·표 · 산업단지·계통 레이어 · 시나리오 증분 지도 |
| 정책 시나리오 | `scenarios.html` | R0–R3 요약·세부 표 · 시군별 조회 · 조건 조합 산정 · 시행령 제정안 영향(`#decree`) |
| 지역별 우선 검토 후보 | `regions.html` | 시군별 비지배 집합 · 가중 병기 순위 · 10방법 비교 · 지역 전력수요 비교 · 제도 경로 |
| 근거와 방법 | `method.html` | 판정 조건 · 데이터 처리 규칙(`#datarules`) · 재현·검증 4종 · 한계(`#caveats`) · 계보 |

구 탭은 리다이렉트 스텁으로만 남아 있다(링크 보호):
`proximity.html`·`map.html`·`candidates.html` → 후보지 찾기 ·
`evidence.html`·`estimate.html` → 정책 시나리오 · `decree.html` → 정책 시나리오 `#decree` ·
