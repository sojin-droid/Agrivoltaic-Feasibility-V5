...
| F14-b | 계통 슬라이더 "제일 왼쪽 포화 → 제일 오른쪽 50MW 이상", 범례와 일치 | 반대 방향(왼쪽 = ≥50MW만 · 오른쪽 = 전체) — 범례 좌→우(포화→50+)와 불일치 | 슬라이더 값 = 문턱으로 직결(`gridT = value`), 초기값 50(오른쪽 끝), 양끝 라벨 "◀ 전체(포화 포함) / 여유 ≥ 50MW만 ▶" | 초기 val=50 · 라벨 "잔여 ≥ 50 MW 읍면동만 채색 — 245곳" · dim 5구간 |
